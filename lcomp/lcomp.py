...
class IDaqLDevice(c_void_p):
    ...
    def __call__(self, *args):
        ...
        if ret and self.name not in {"CallCreateInstance", "OpenLDevice", "Get_LDEV2_Interface"}:
...
class LCOMP(object):
    ...
    def CreateInstance(self, slot):
        ...
        if not result:
            msg = "CreateInstance error {} ({})".format(err.value,
...
# Расширенный интерфейс для работы с устройствами
    ...
    def Get_LDEV2_Interface(self):
        ...
        if not result:
            _logger.error("Get_LDEV2_Interface error %d (%s)", err.value,
                                                         ERROR_CODE[err.value])
            return None
        self._ifc2 = _ifc_type(result)
        return bool(self._ifc2) or None

    def Release_LDEV2_Interface(self):
        """Закрыть расширенный интерфейс."""

        return not self._ldev.Release_LDEV2_Interface(self._ifc2) or None

    def InitStartLDeviceEx(self, stream_id):
        """Функция инициализирует внутренние переменные драйвера перед началом сбора."""

        return not self._ldev.InitStartLDeviceEx(self._ifc2, c_uint(stream_id)) or None

    def StartLDeviceEx(self, stream_id):
        """Функция запускает сбор данных с платы в большой кольцевой буфер."""

        return not self._ldev.StartLDeviceEx(self._ifc2, c_uint(stream_id)) or None

    def StopLDeviceEx(self, stream_id):
        """Функция останавливает сбор данных с платы в большой кольцевой буфер."""

        return not self._ldev.StopLDeviceEx(self._ifc2, c_uint(stream_id)) or None
